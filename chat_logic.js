document.addEventListener('DOMContentLoaded', () => {
    ...
    const saveState = () => localStorage.setItem('neuronix_chat_state_final_v4', JSON.stringify(state));
    const loadState = () => {
        const saved = localStorage.getItem('neuronix_chat_state_final_v4');
        if (saved) {
    ...
    const renderChat = () => {
        ...
        if (activeChat && activeChat.messages.length > 0) {
            activeChat.messages.forEach(msg => appendMessage(msg.sender, msg.content, false)); // Don't save on initial render
    ...
    function createActionToolbar(messageText) {
        ...
        regenBtn.onclick = () => { regeneratePopup.classList.remove('hidden'); };
        toolbar.append(copyBtn, likeBtn, dislikeBtn, speakBtn, regenBtn);
        return toolbar;
    }
    
    regenerateConfirmBtn.onclick = () => { regeneratePopup.classList.add('hidden'); if(state.lastUserQuery){ regenerateResponse(); } };
    ...
    async function regenerateResponse() {
        ...
        renderChat();
        const aiBubble = appendMessage('ai', '<span class="loader"></span>', false); // Error ko save nahi karna
        try {
            // ... (baaki logic same)
        } catch (error) {
            const errorMsg = `System Error: ${error.message}`;
            aiBubble.innerHTML = errorMsg;
            // Yahan se addMessageToHistory hata diya
        }
    }
    ...
    async function transmitQuery() {
        ...
        try {
            ...
            const errorMsg = `System Error: ${error.message}`;
            aiBubble.parentElement.remove();
            appendMessage('ai', errorMsg, false); // Error ko save nahi karna
        } finally {
    ...
    const applyTheme = (isLight) => {
        ...
        localStorage.setItem('neuronix_theme_light_final_v4', isLight);
    ...
    renderChat();
    renderSidebar();
    applyTheme(JSON.parse(localStorage.getItem('neuronix_theme_light_final_v4') || 'false'));
    thinkingModeBtn.classList.toggle('active', state.isThinkingMode);
});
